...
from qa_system import QASystem, interactive_chat
...
def start_qa_system(vector_store, bm25_instance, all_splits):
    """启动问答系统"""
    # 检查API密钥
    ...
    # 启动交互式聊天
    interactive_chat(vector_store, bm25_instance, all_splits)

if __name__ == "__main__":
    root_folder = "MinerU"
    index_path = "save/faiss_index_only"
    splits_file = "save/splits_data.json"
    bm25_corpus_file = "save/bm25_corpus_data.json"
    ...
    if os.path.exists(index_file_path) and os.path.exists(splits_file) and os.path.exists(bm25_corpus_file):
        ...
        try:
            ...
            # vector_store = vector_store.to_device(0)
    ...
    start_qa_system(vector_store, bm25_instance, all_splits)